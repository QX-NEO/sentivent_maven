...
def get_aux_input_dict(path, tokenizer):
    ...
    _label_input_mask = [1 for lii in _label_input_ids]
    _label_segment_ids = [0 for lii in _label_input_ids]
    _label_token_lens = []
    for lt in _labels:
    ...
    _aux_input_dict = {
        ...
        "label_segment_ids": torch.tensor(_label_segment_ids, dtype=torch.long),
        "_label_token_lens": _label_token_lens
    }
...
def main():
    ...
    # Setup distant False if needed
    if args.server_ip and args.server_port:
        # Distant False - see https://code.visualstudio.com/docs/python/False#_attach-to-a-local-script
        import ptvsd
    ...
    if args.do_train:
        ...
        print("train data: ", train_dataset.tensors)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            print("checkpoint label ", len(labels))
                ...
                for line in lines:
                    ...
                    for mention in doc['candidates']:
                        ...
                        if not is_NA:
                            ...
                            #     res['predictions'].append({"id":mention['id'],"type_id":mavenTypes.index('Process_start')})
                            # else:
                            #     res['predictions'].append({"id":mention['id'],"type_id":mavenTypes.index(Type)})
                            #     print("ignoring I-Type")
                            #     print("tokens\t", doc['content'][mention['sent_id']]['tokens'], "\n")
