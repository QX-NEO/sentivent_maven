    ...
    with open(file_path, "r") as fin:
        ...
        for line in lines:
            ...
            for sent in doc['content']:
                ...
                labels.append(
                    ['O' for i in range(0, len(sent['tokens']))])  # TBD
            if mode != 'test':
                for event in doc['events']:
                    for mention in event['mention']:
                        labels[mention['sent_id']][mention['offset']
                                                   [0]] = "B-" + event['type']
                        for i in range(mention['offset'][0] + 1, mention['offset'][1]):
                            labels[mention['sent_id']][i] = "I-" + \
                                event['type']
...
def convert_examples_to_features(examples,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        # print( "label length ", len(label_ids))
        # print("max_seq_length ", max_seq_length)
        # print("special_tokens_count",special_tokens_count)
        # print("padding_length", padding_length)
        # print('pad_token_label_id',pad_token_label_id)
        # print(label_ids)
        # label_id2token = tokenizer.convert_ids_to_tokens(label_ids)
        # input_id2token = tokenizer.convert_ids_to_tokens(input_ids)
        # print(label_id2token)
        # print('input_mask_len',len(input_mask))
        # print(input_id2token)
        # print(len(input_id2token))
...
def get_labels(path):
    ...
    # return ['O', 'B-Gene_expression', 'I-Gene_expression', 'B-Transcription', 'I-Transcription', 'B-Localization', 'I-Localization', 'B-Protein_catabolism', 'I-Protein_catabolism', 'B-Binding', 'I-Binding', 'B-Process', 'I-Process', 'B-Phosphorylation', 'I-Phosphorylation', 'B-Regulation', 'I-Regulation', 'B-Positive_regulation', 'I-Positive_regulation', 'B-Negative_regulation', 'I-Negative_regulation']
...
def get_label_seed_list(path):
    ...
    # return ["Gene_expression", "Transcription", "Localization", "Protein_catabolism", "Binding", "Process", "Phosphorylation",  "Regulation", "Positive_regulation", "Negative_regulation"]
